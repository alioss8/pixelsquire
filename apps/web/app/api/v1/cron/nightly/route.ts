...
async function processSub(sub: Sub, now: Date) {
  ...
  if (localHour === SUMMARY_HOUR) {
    ...
    if (completedToday === 0) {
      const streak = await calcStreak(sub.device.userId);
      if (streak > 0) {
        await sendPush(
          sub,
          "PixelSquire ⚔️",
          `${streak} günlük serin tehlikede kral! Bugün henüz hiç quest tamamlamadın, gece bitmeden bir şey yap yoksa sıfırlanır.`,
        );
      }
      return;
    }
  ...
  if (NUDGE_HOURS.includes(localHour)) {
    ...
    if (totalGoals > 0 && completedToday >= totalGoals) return; // bugün zaten bitirmiş, nagging yapma
...
export async function POST(request: NextRequest) {
  ...
  await Promise.all(subs.map((sub) => processSub(sub, now)));
