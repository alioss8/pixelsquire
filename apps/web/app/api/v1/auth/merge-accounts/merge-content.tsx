...
export function MergeContent() {
  ...
  const handleMerge = async (keepUserId: string) => {
    const res = await fetch('http://localhost:3001/api/v1/auth/merge', {
      method: 'POST',
  ...
  return (
    <div style={{ textAlign: 'center', padding: '40px' }}>
      ...
      <button onClick={() => handleMerge(googleUserId)}>Keep Google Account</button>
      <button onClick={() => handleMerge(anonUserId)}>Keep Anonymous Account</button>
    </div>
  )
}